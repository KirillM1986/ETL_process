...
CREATE TABLE DM.DM_F101_ROUND_F(
	...
	CONSTRAINT PK_LEDGER_ACCOUNT PRIMARY KEY (LEDGER_ACCOUNT, FROM_DATE)
);
...
BEGIN
    CALL logs.start_logs('2018-02-01'::DATE, 'dm.dm_f101_round_f', 'Расчет формы 101', start_time);
    CALL dm.fill_f101_round_f('2018-02-01'::DATE);
    CALL logs.end_logs('dm.dm_f101_round_f', start_time,  CLOCK_TIMESTAMP(), 'Данные успешно загружены');
END $$;
