...
BEGIN
    FOR i IN 0..30 LOOP
		...
		CALL logs.start_logs(OnDate+i, 'dm.dm_account_turnover_f', 'Загрузка витрины оборотов', start_time);
        CALL ds.fill_account_turnover_f(OnDate+i);
		CALL logs.end_logs('dm.dm_account_turnover_f', start_time,  CLOCK_TIMESTAMP(), 'Данные успешно загружены');
    END LOOP;
END $$;
...
BEGIN
    FOR i IN 0..30 LOOP
		start_time = CLOCK_TIMESTAMP();
		CALL logs.start_logs(OnDate+i, 'dm.dm_account_balance_f', 'Загрузка витрины остатков', start_time);
        CALL ds.fill_account_balance_f(OnDate + i);
		CALL logs.end_logs('dm.dm_account_balance_f', start_time,  CLOCK_TIMESTAMP(), 'Данные успешно загружены');
    END LOOP;
END $$;
